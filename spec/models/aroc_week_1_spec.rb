...
# You can comment out the Ruby example after your AR is working. 
  ...
  # ------------------ Using ActiveRecord ----------------------
    order_id = Order.all.order(amount: :asc).pluck(:id).first
  ...
  it '3. finds order id of largest order' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    order_id = Order.all.order(amount: :desc).pluck(:id).first
    # ------------------------------------------------------------
  ...
  it '4. finds orders of multiple amounts' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    orders_of_500_and_700 = Order.all.where(amount: [500, 700])
    orders_of_700_and_1000 = Order.all.where(amount: [700, 1000])
    # ------------------------------------------------------------
