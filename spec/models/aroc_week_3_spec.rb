...
# ----------------------
  ...
  it '16. returns the names of users who ordered one specific item' do
    ...
    users = User.select(:name).distinct.joins(:order_items).where(order_items: { item_id: @item_8} ).pluck(:name)
    # ------------------------------------------------------------
  ...
  it '18. returns the names of items for a users order' do
    ...
    items_for_user_3_third_order = Order.where(user_id: @user_3).third.items.pluck(:name)
    # ------------------------------------------------------------
  ...
  it '20. returns the average amount for all orders for one user' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    average = Order.where(user_id: @user_3).average(:amount)
    # ------------------------------------------------------------
